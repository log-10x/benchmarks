...
**Against the right control the typed layout is not a saving.** Take the folded native table and simply drop its text index: 7,570,122 minus 3,385,853 is 4,184,269 bytes, arithmetic rather than a rebuilt table. The typed layout is 4,028,701 bytes, 3.7% smaller than that, and 4,376,741 with its dictionary, 4.6% larger. The type-sorted typed layout is 3,474,093 bytes, 17.0% smaller. Every headline saving in the totals table is the index leaving, and a customer can drop an index with one statement.
